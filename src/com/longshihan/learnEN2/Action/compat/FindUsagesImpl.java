...
class FindUsagesImpl {
    ...
    int findUsage(PsiElement element) {
        final AtomicInteger mCount = new AtomicInteger(0);
        ...
        try {
            FindUsagesHandler handler = this.getFindUsagesHandler(element);
            if (handler != null) {
    ...
    private FindUsagesHandler getFindUsagesHandler(@NotNull PsiElement element) {
        ...
        for(int var5 = 0; var5 < var4; ++var5) {
            ...
            if(factory.canFindUsages(element)) {
                FindUsagesHandler handler = factory.createFindUsagesHandler(element, false);
    ...
    private static UsageSearcher createUsageSearcher(@NotNull PsiElement2UsageTargetAdapter[] primaryTargets, @NotNull PsiElement2UsageTargetAdapter[] secondaryTargets, @NotNull FindUsagesHandler handler, @NotNull FindUsagesOptions options, PsiFile scopeFile) throws PsiInvalidElementAccessException {
        ReadAction.run(() -> {
            PsiElement[] primaryElements = convertToPsiElements(primaryTargets);
            PsiElement[] secondaryElements = convertToPsiElements(secondaryTargets);
        ...
        return (processor) -> {
            PsiElement[] primaryElements = (PsiElement[]) ReadAction.compute(() -> {
                return convertToPsiElements(primaryTargets);
            });
            PsiElement[] secondaryElements = (PsiElement[])ReadAction.compute(() -> {
                return convertToPsiElements(secondaryTargets);
            });
            ...
            Processor<UsageInfo> usageInfoProcessor = new CommonProcessors.UniqueProcessor<UsageInfo>(new Processor<UsageInfo>() {
                @Override
